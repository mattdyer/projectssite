...
?>
	<script type="text/javascript">
		...
		function Start(){
			...
			for(Type in Species){
				ControlArea.innerHTML+='<div onclick="AddAnimal(\'' + Type + '\',0,0,true,false)" style="cursor:pointer;">' + Type + '</div>';
			}
		}
		...
		function AddAnimal(Type,Top,Left,RandomPosition,HasParent,ParentID){
			...
			MoveAnimal(Type + NewAnimalID);
		}
		
		function MoveAnimal(AnimalID){
			if(document.getElementById(AnimalID))
			{
				var ThisAnimal = document.getElementById(AnimalID);
				var ThisAnimalImage = document.getElementById(AnimalID + 'Image');
				...
				if(!ThisAnimalInfo['Dead'])
				{
					...
					if(ThisAnimalInfo['Life'] >= ThisSpeciesInfo['ReproduceLife'])
					{
						...
						var ReproduceTop = ThisAnimalInfo['Top'] + Math.floor((Math.random() * (ThisAnimalInfo['Height'] * 3)) - (ThisAnimalInfo['Height'] * 1.5));
						var ReproduceLeft = ThisAnimalInfo['Left'] + Math.floor((Math.random() * (ThisAnimalInfo['Width'] * 3)) - (ThisAnimalInfo['Width'] * 1.5));
						...
						AddAnimal(ReproduceType,ReproduceTop,ReproduceLeft,false,true,AnimalID);
					}
					...
					AnimalInfo[AnimalID]['Left'] = NewLeft;
					ThisAnimal.style.top = NewTop + 'px';
					AnimalInfo[AnimalID]['Top'] = NewTop;
						
					setTimeout('MoveAnimal(\'' + AnimalID + '\')',100);
				}
				else
				{
					ThisAnimal['Life'] = ThisSpeciesInfo['DeadLife'];
					setTimeout('RemoveAnimal(\'' + AnimalID + '\')',20000);
				}
			}
		}
